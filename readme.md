# 貪食蛇走迷宮
+ 課程出發點為利用 OLED 點亮後，不進行刷新。記錄軌跡後存成 list 格式，若碰觸到自己軌跡則遊戲結束，並且計算走過軌跡點之數量，學習檔案存取。
+ 課程第二階段加入迷宮繪製，將迷宮併入軌跡，以條件式判斷點是否位於 list 中。將來課程可發展成專案，改為兩個玩家同時進行，各自操控所屬進行對戰。
+ 標注頁碼的部分，來自中央大學吳維漢教授著作【簡明python學習講義】，提供教學輔助用
## 硬體組裝
1. 工具
	+ 斜口鉗、尖嘴鉗、撥線鉗
	+ 銲槍、銲錫
...
0. pinTest 硬體測試
	...
	+ 按鈕
	+ 蜂鳴器
1. button_thread，雙執行緒：按鈕只兩個，利用一個 thread 讀取方向、主程式顯示方向(全域變數 global 參見 p.227)
	1. 利用按鈕決定方向
...
1. 讀取檔案
	...
	+ list
		+ len(a)
		+ a[0]
		+ .split(,)(參見 .rstrip p.137, .split p.161)
1. 失敗了。就爆炸吧
	+ 定義函式
	+ 繪圖
...
1. 計算分數
	1. 當座標所在 in path:( format 方式參見 p.30)
		1. 蜂鳴器叫
		3. 遊戲結束
	2. 當抵達終點：
		1. 好棒棒
1. 不只一個迷宮時，使用 for 迴圈直接按順序下去
	1. 設定迷宮初始檔名值
	2. 沒抵達終點時，使用 contiune，進行下一次迭代繼續重複(contiune 參見p.30)
